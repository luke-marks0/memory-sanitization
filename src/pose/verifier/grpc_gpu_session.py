...
from pose.verifier.gpu_planning import (
    build_gpu_session_plan,
    detect_gpu_memory_bytes,
    validate_single_gpu_session_plan,
)
...
from pose.verifier.host_planning import build_region_sector_plan
from pose.verifier.outer import decode_opening_payload, verify_outer_challenge_response
from pose.verifier.session_store import sessions_root


def _measure_gpu_unit_size(
    *,
    profile: BenchmarkProfile,
    verifier: VendoredFilecoinReference,
) -> int:
    unit = verifier.seal_porep_unit(
        storage_profile=profile.porep_unit_profile,
        leaf_alignment_bytes=profile.leaf_size,
    )
    return len(unit.serialized_bytes)


def run_gpu_session_via_grpc(
    profile: BenchmarkProfile,
    *,
    ...
    gpus = [int(device) for device in profile.target_devices.get("gpus", [])]
    if profile.target_devices.get("host", False) or len(gpus) != 1:
        result.verdict = "PROTOCOL_ERROR"
        result.notes.append("Phase 2 gpu runner only supports one gpu-only region.")
        result.timings_ms["total"] = int((perf_counter() - total_started) * 1000)
        return result
    if retain_session:
        result.verdict = "PROTOCOL_ERROR"
        result.notes.append("Retained resident sessions are not yet supported for gpu runs.")
        result.timings_ms["total"] = int((perf_counter() - total_started) * 1000)
        return result
    ...
    if session_plan is None:
        ...
        measured_unit_size = _measure_gpu_unit_size(profile=profile, verifier=verifier)
        session_plan, gpu_plan = build_gpu_session_plan(
            profile,
            session_id=result.session_id,
            session_nonce=result.session_nonce,
            device=device,
            unit_size_bytes=measured_unit_size,
            detected_gpu_bytes=detect_gpu_memory_bytes(device),
        )
        validate_single_gpu_session_plan(session_plan)
        region = session_plan.regions[0]
        region_id = region.region_id
        device = int(region.gpu_device)
        planned_region_bytes = region.usable_bytes
        gpu_total_bytes = gpu_plan.total_bytes
    else:
        try:
            validate_single_gpu_session_plan(session_plan)
        ...
        region_id = region.region_id
        planned_region_bytes = region.usable_bytes
        _available_bytes, gpu_total_bytes = detect_gpu_memory_bytes(device)
    ...
    result.notes.append(
        f"HBM-only session over {session_plan.unit_count} canonical PoRep unit(s) on gpu {device}."
    )
    ...
    try:
        ...
        tracker.values["copy_to_hbm"] = int(worker_timings.get("copy_to_hbm", 0))
        ...
        result.gpu_devices = [device]
        result.gpu_usable_bytes_by_device = {str(device): region_manifest.usable_bytes}
        result.gpu_covered_bytes_by_device = {str(device): region_manifest.payload_length_bytes}
        ...
        result.coverage_fraction = (
            region_manifest.payload_length_bytes / region_manifest.usable_bytes
            if region_manifest.usable_bytes
            else 0.0
        )
        ...
        result.notes.append(f"Detected gpu total bytes on device {device}: {gpu_total_bytes}")
        ...
        if not lease_closed:
            ...
            try:
                if remote_cleanup_ran:
                    ...
                    result.cleanup_status = release_gpu_lease(
                        lease,
                        zeroize=bool(cleanup_policy["zeroize"]),
                        verify_zeroization=bool(cleanup_policy["verify_zeroization"]),
                    )
