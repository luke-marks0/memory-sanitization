...
def _write_hybrid_profile(path: Path) -> None:
    path.write_text(
        """
name: bad-hybrid-memory-accounting
benchmark_class: cold
target_devices:
  host: true
  gpus: [0]
...
def test_prepare_calibration_counts_uncovered_targeted_host_and_gpu_tiers_for_hybrid_profiles(
    monkeypatch,
    tmp_path: Path,
) -> None:
    profile_path = tmp_path / "profile.yaml"
    artifact_dir = tmp_path / "artifacts"
    _write_hybrid_profile(profile_path)
    ...
    monkeypatch.setattr(
        ...
        lambda profile: plan_slot_layout(
            profile,
            detected_host_bytes=262_144,
            detected_gpu_bytes_by_device={0: (131_072, 262_144)},
        ),
    )
    monkeypatch.setattr("pose.benchmarks.calibration.get_cuda_runtime", lambda: _HybridRuntime())
    monkeypatch.setattr(
        "pose.benchmarks.calibration._measure_resident_lookup_latency_us_by_region",
        lambda **_kwargs: {
            "host-0": {"mean": 10.0, "p50": 10.0, "p95": 12.0, "p99": 14.0, "max": 14.0},
            "gpu-0": {"mean": 11.0, "p50": 11.0, "p95": 13.0, "p99": 15.0, "max": 15.0},
        },
    )
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: {
            ...
            "gpu-0": {
                "fast_phase_round_trip_us": {"mean": 92.0, "p50": 90.0, "p95": 102.0, "p99": 112.0, "max": 112.0},
                "fast_phase_prover_lookup_round_trip_us": {"mean": 9.0, "p50": 9.0, "p95": 11.0, "p99": 13.0, "max": 13.0},
                "fast_phase_transport_overhead_us": {"mean": 83.0, "p50": 81.0, "p95": 91.0, "p99": 99.0, "max": 99.0},
            },
        },
    )
    monkeypatch.setattr(
    ...
    assert payload["status"] == "calibration-invalid"
    assert payload["planning"]["effective_attacker_budget_bytes_assumed"] > payload["planning"]["base_attacker_budget_bytes_assumed"]
    assert any("targeted_local_host_tier_uncovered=" in note for note in payload["planning"]["claim_notes"])
    assert any("targeted_local_gpu_tier_uncovered=" in note for note in payload["planning"]["claim_notes"])
