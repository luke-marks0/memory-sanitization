...
The repository must not reintroduce a separate “inner proof / outer proof” split. The graph-based PoSE-DB protocol itself is the required proof mechanism.
...
2. One seeded labelling phase plus timed single-label rounds.
   The protocol is not split into a separate inner proof and outer proof.
